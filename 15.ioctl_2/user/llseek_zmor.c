...
static long llseek_device_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    {
        memset(dev_handle->kernel_buffer, 0, sizeof(dev_handle->kernel_buffer));
        printk(KERN_INFO "kernel_buffer is reset\n");
        break;
    }
    ...
    case IOCTL_GET_BUFFER_SIZE:
    {
        ...
        if (copy_to_user((void __user *)arg, &buffer_size, sizeof(buffer_size)))
        {
            return -EFAULT;
        }
        break;
    ...
    {
        int position = 0;
        if (copy_from_user(&position, (void __user *)arg, sizeof(position)))
        {
            return -EFAULT;
        }
        loff_t ret_pos = llseek_device_llseek(filp, position, SEEK_SET);
        if (ret_pos < 0)
        {
            return ret_pos; // 返回错误码
        }
        printk(KERN_INFO "Position set to %d, filp->f_pos = %lld\n", position, filp->f_pos);
        break;
    }
    case IOCTL_GET_POSITION:
    {
        loff_t now_position = filp->f_pos;
        if (copy_to_user((void __user *)arg, &now_position, sizeof(now_position)))
        {
            return -EFAULT;
        }
        printk(KERN_INFO "Current position is %lld, filp->f_pos = %lld\n", now_position, filp->f_pos);
        break;
    }
    case IOCTL_FILL_BUFFER:
    {
        char fill_char;
        if (copy_from_user(&fill_char, (void __user *)arg, sizeof(fill_char)))
        {
            return -EFAULT;
        }
        memset(dev_handle->kernel_buffer, fill_char, sizeof(dev_handle->kernel_buffer));
        printk(KERN_INFO "Buffer filled with character '%c'\n", fill_char);
        break;
    }
    case IOCTL_TRANSF_STRUCT:
    {
        trandata_t data_example;
        strncpy(data_example.cmd, "zmor", sizeof(data_example.cmd) - 1);
        data_example.cmd[sizeof(data_example.cmd) - 1] = '\0';
        data_example.length = 2;
        
        if (copy_to_user((void __user *)arg, &data_example, sizeof(data_example)))
        {
            return -EFAULT;
        }
