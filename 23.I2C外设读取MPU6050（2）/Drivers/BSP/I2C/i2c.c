...
void I2C_SendByte(uint16_t adr, uint16_t memAdr, uint8_t data)
{
    ...
    HAL_I2C_Mem_Write(&hi2c2, adr, memAdr, I2C_MEMADD_SIZE_8BIT, &data, 1, 1000);
}

/* 向从设备发送多个字节数据 */
void I2C_SendBytes(uint16_t adr, uint8_t *sendBuffer, uint8_t len)
{
    HAL_I2C_Master_Transmit(&hi2c2, adr, sendBuffer, len, 1000);
}
...
void I2C_RecieveByte(uint16_t adr, uint16_t memAdr, uint8_t *data)
{
    ...
    HAL_I2C_Mem_Read(&hi2c2, adr, memAdr, I2C_MEMADD_SIZE_8BIT, data, 1, 1000);
}