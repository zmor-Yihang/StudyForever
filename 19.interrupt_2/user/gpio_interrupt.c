...
/* 传给中断下文回调函数的参数 */
unsigned long tasklet_data = 1;

/* Tasklet结构体, 中断下文结构体 */
struct tasklet_struct tasklet_struct;
...
static int __init gpio_interrupt_init(void)
{
    ...
    /* 初始化中断下文结构体 */
    tasklet_init(&tasklet_struct, tasklet_callback, tasklet_data);
