...
//
// LSPCLK = SYSCLKOUT / 4 = 150MHz / 4 = 37.5MHz（由 InitSysCtrl 的
// LOSPCP=0x0002 决定） 波特率分频值 BRR = LSPCLK / (baud * 8) - 1，拆成高/低 8
// 位写入 SCIHBAUD/SCILBAUD
//
...
// TX：FIFO 里剩 <= 该值个字节触发发送中断。设 0 = 发空才补，约每 16
// 字节一次中断
//
