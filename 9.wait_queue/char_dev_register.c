...
static ssize_t cdev_read(struct file *filp, char __user *user_read_buffer, size_t count, loff_t *f_pos)
{
    ...
    /* 等待阻塞标志位被设置 */
    wait_event_interruptible(queue_head_of_cdev, device_handle->flag);
...
module_init(char_dev_register_init); /* 加载字符设备模块时调用 */
module_exit(char_dev_register_exit); /* 卸载字符设备模块时调用 */
