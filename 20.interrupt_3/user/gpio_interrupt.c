...
static struct work_struct gpio_work_struct;

/* 中断下文-工作队列回调函数 */
static void work_callback(struct work_struct *ptr)
{
...
static irqreturn_t gpio_interrupt_callback(int irq, void *dev_id)
{
    ...
    /* 调度工作队列 */
    schedule_work(&gpio_work_struct);
...
static int __init gpio_interrupt_init(void)
{
    ...
    /* 注册中断 */
    int ret = request_irq(gpio_irq_number, gpio_interrupt_callback, IRQF_TRIGGER_FALLING, "gpio_interrupt", NULL);
    if (ret)
    {
    ...
    /* 初始化工作队列 */
    INIT_WORK(&gpio_work_struct, work_callback);
