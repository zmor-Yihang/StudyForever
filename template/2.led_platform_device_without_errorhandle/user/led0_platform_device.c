#include "led0_platform_device.h"

/* 设备释放函数声明 */
void led0_platform_device_release(struct device *dev);
...
struct platform_device led0_platform_device_t = {
    .name = "led0",                              // 设备名称，用于匹配驱动
    .id = -1,                                    // 设备ID，-1表示只有一个此类设备
    .resource = led0_resources,                  // 设备使用的资源数组
    .num_resources = 3,                          // 资源数量
    .dev.release = led0_platform_device_release, // 设备释放回调函数
};
...
void led0_platform_device_release(struct device *dev)
{
    printk("This is led0_platform_device_release!\n");
}

static int __init led_platform_device_init(void)
{
    /* 注册平台设备到内核中 */
    platform_device_register(&led0_platform_device_t);
...
{
    /* 从内核中注销平台设备 */
    platform_device_unregister(&led0_platform_device_t);
    printk("LED platform device led0 unregistered\n");
}
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("zmor");
MODULE_DESCRIPTION("A platform led0 device module");