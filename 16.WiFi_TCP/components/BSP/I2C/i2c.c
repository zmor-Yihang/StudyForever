...
void i2c_add_device(uint8_t device_address, i2c_master_dev_handle_t *dev_handle)
{
    ...
    device_config.device_address = device_address;      /* 纯设备地址, 不包含读写位, IDF会自动左移并添加读写位 */
 ...
 */
void i2c_transmit_data(i2c_master_dev_handle_t dev_handle, uint8_t *data, size_t data_len)
{
    i2c_master_transmit(dev_handle, data, data_len, 5000);
}
 ...
 */
void i2c_receive_data(i2c_master_dev_handle_t dev_handle, uint8_t *data, size_t data_len)
{
    i2c_master_receive(dev_handle, data, data_len, 5000);
}
 ...
 */
void i2c_transmit_receive_data(i2c_master_dev_handle_t dev_handle, 
                           uint8_t *tx_data, size_t tx_data_len, 
                           uint8_t *rx_data, size_t rx_data_len)
{
    i2c_master_transmit_receive(dev_handle, tx_data, tx_data_len, rx_data, rx_data_len, 5000);
}