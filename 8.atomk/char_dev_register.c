...
static int cdev_open(struct inode *inode, struct file *filp)
{
    ...
    /* 检查设备是否已被打开 - 使用原子操作确保线程安全 */
    if (atomic64_dec_and_test(&handle->open_count) == 0) {
        /* 如果返回0，说明open_count在减1前大于1，设备已被打开 */
        printk(KERN_INFO "cdev_open: device %d:%d is already open\n", 
               MAJOR(handle->device_number), MINOR(handle->device_number));
        atomic64_inc(&handle->open_count); /* 恢复计数 */
        return -EBUSY;
    }
...
static int __init char_dev_register_init(void)
{
    ...
    for (i = 0; i < CDEV_NUM; i++)
    {
        ...
        /* 初始化打开计数 */
        atomic64_set(&hcdev[i].open_count, 1);
