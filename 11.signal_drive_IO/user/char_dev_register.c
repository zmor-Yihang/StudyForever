...
/* 虚拟地址映射指针 */
void __iomem *pmu_grf_base_addr = NULL; /* PMU GRF基地址虚拟映射 */
void __iomem *gpio_0_base_addr = NULL;  /* GPIO_0基地址虚拟映射 */

/* 定义等待队列头, 该队列被所有字符设备实例共享, 由内核维护 */
DECLARE_WAIT_QUEUE_HEAD(queue_head_of_cdev);
...
static __poll_t cdev_poll(struct file *filp, struct poll_table_struct *wait);
...
static int cdev_open(struct inode *inode, struct file *filp)
{
    ...
    /* 检查设备是否已被打开 */
    if (atomic64_read(&handle->open_count) == 0)
    {
        printk(KERN_INFO "cdev_open: device %d:%d is already open\n",
               MAJOR(handle->device_number), MINOR(handle->device_number));
        return -EBUSY;
    }
    atomic64_dec(&handle->open_count);
...
static ssize_t cdev_read(struct file *filp, char __user *user_read_buffer, size_t count, loff_t *f_pos)
{
    ...
    printk(KERN_INFO "cdev_read: read %zu bytes from device %d:%d, buffer content: %s\n",
           to_read, MAJOR(device_handle->device_number), MINOR(device_handle->device_number), device_handle->kernel_buffer);
...
static ssize_t cdev_write(struct file *filp, const char __user *user_write_buffer, size_t count, loff_t *f_pos)
{
    ...
    if (strcmp(device_handle->kernel_buffer, "on") == 0)
    {
        ...
        wake_up_interruptible(&queue_head_of_cdev);
        /* 发送信号给注册的进程 */
        if (device_handle->fasync_queue)
            kill_fasync(&device_handle->fasync_queue, SIGIO, POLL_IN);
    }
...
static __poll_t cdev_poll(struct file *filp, struct poll_table_struct *wait)
{
    ...
    poll_wait(filp, &queue_head_of_cdev, wait);
...
static int cdev_fasync(int fd, struct file *filp, int mode)
{
    ...
    return fasync_helper(fd, filp, mode, &device_handle->fasync_queue);
}
...
static int __init char_dev_init(void)
{
    ...
    for (i = 0; i < CDEV_NUM; i++)
    {
        ...
        /* 初始化打开计数 */
        atomic64_set(&hcdev[i].open_count, 2);

        /* 初始化状态标志位 */
        hcdev[i].flag_of_poll = 0;

        /* 初始化异步通知队列 */
        hcdev[i].fasync_queue = NULL;
    ...
    {
        ret = -ENOMEM;
        goto err_unmap_pmu_grf;
    }

    /* 映射其它设备的IO地址, 不同设备需要不同的映射 */
...
static void __exit char_dev_exit(void)
{
    ...
    hcdev = NULL;
    cdev_class_p = NULL;
    gpio_0_base_addr = NULL;
    pmu_grf_base_addr = NULL;

    printk("Goodbye cdev example!\n");
}

module_init(char_dev_init); /* 加载字符设备模块时调用 */
module_exit(char_dev_exit); /* 卸载字符设备模块时调用 */
